(function(k, h) {
    function D() {
        function b() {
            ...
            q = h.innerHeight;
            l.setSize(p, q);
            e.aspect = h.innerWidth / h.innerHeight;
        ...
        c.fog = new THREE.FogExp2(16777215, .045);
        e = new THREE.PerspectiveCamera(45, p / q, .1, 1E4);
        e.position.z = 10;
        e.position.y = 5;
        e.lookAt(d.position);
        c.add(e);
        ...
        c.add(a);
        a = [];
        a[0] = new THREE.DirectionalLight(16777215, .5);
        a[1] = new THREE.DirectionalLight(16777215, .5);
        a[2] = new THREE.DirectionalLight(16777215, .5);
        ...
        c.add(r);
        for (var w = h.devicePixelRatio / 1.5 * 40, a = new THREE.Geometry, w = new THREE.ShaderMaterial({
                uniforms: {
                    color: {
                        type: "c",
                ...
                vertexShader: k.getElementById("vs").textContent,
                fragmentShader: k.getElementById("fs").textContent,
                blending: THREE.AdditiveBlending,
                transparent: !0,
                depthTest: !1
            }), A = 0; 1E4 > A; A++) {
            var g = new THREE.Vector3(100 * (Math.random() - .5), 100 * Math.random(), 100 * (Math.random() - .5));
            a.vertices.push(g)
        }
    ...
    function E() {
        (new THREE.ObjectLoader).load("./assets/models/tree.min.json", function(b) {
            c = b;
            d = c.children[0];
            ...
            (new THREE.TextureLoader).load("./assets/snowflake.png", function(a) {
                x = a;
    ...
    function B() {
        ...
        var b = C.get(["buffer", "rms"]);
        y.getDelta();
        var a = y.getElapsedTime();
        n.rotation.y -= .015;
        e.position.x = d.position.x + 10 * Math.cos(.06 * a);
        e.position.z = d.position.z + 10 * Math.sin(.06 * a);
        e.lookAt(d.position);
    ...
    function F(b) {
        ...
        function a() {
            var a = k.querySelector(".tracklist li.active");
            a && a.classList.remove("active");
            k.querySelectorAll(".tracklist li")[f.playlistIndex].classList.add("active")
        }
        var e = k.getElementsByClassName("tracklist")[0],
            c = [];
        b.forEach(function(a, b) {
            var d = k.createElement("li");
    ...
    var c, e, l, d, v, z, r, n, C, m, t, y, x, p, q, f, u = [];
    (function() {
        request = new XMLHttpRequest;
        request.open("GET", "./assets/json/tracklist.min.json", !0);
        request.onload = function() {
            200 <= request.status && 400 > request.status ? (data = JSON.parse(request.responseText), F(data)) : console.error(request)
        };
