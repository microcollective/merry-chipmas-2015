function ThinPlayer(settings) {
	...
	if(typeof id3 != 'undefined') {
		useid3 = true;
		console.log('Detected what seems to be the id3 library - gonna try to use it.');
	}
	...
	if('element' in self.settings) {
		...
		container.id	= 'tp-container';	self.settings.bindings.player.container = '#' + container.id;
		playbar.id		= 'tp-playbar';		self.settings.bindings.player.playBar 	= '#' + playbar.id;
		seekbar.id		= 'tp-seekbar';		self.settings.bindings.player.seekBar 	= '#' + seekbar.id;
		loadbar.id		= 'tp-loadbar';		self.settings.bindings.player.loadBar 	= '#' + loadbar.id;
		time.id			= 'tp-time';		self.settings.bindings.player.time 		= '#' + time.id;
		...
		quietMode = true;
		console.log('No element or binding set, running in "quiet" mode (if there is such a thing).');
	}
		...
		if(typeof srcX != 'undefined') {
			...
			if(self.playlist) {
				...
				if(isObject(self.settings.src[self.playlistIndex])) {
					...
					Object.keys(self.settings.src[self.playlistIndex]).forEach(function(key) {
	...
	self.play = function() {
		if(!self.isPlaying()) self.el.play();
...
/* 		}, 50); */
	...
	function makeTime() {
		...
		/* Callback */
		if(typeof self.settings.ontimeupdate === 'function') self.settings.ontimeupdate({currentTime: self.el.currentTime});
	}
	...
	function timeBar() {
		...
		/* Callback */
		if(typeof self.settings.onfasttimeupdate === 'function' && (self.isPlaying() || dragging)) self.settings.onfasttimeupdate({currentTime: self.el.currentTime});
	}
	...
	function loader() {
		if(!quietMode) {
			queryFor(self.settings.bindings.player.loadBar, function(loadBar) {
				...
				var progress;
				try {
					progress = Math.round((self.el.buffered.end(0) / self.el.duration) * 100);
					if(typeof self.settings.progress === 'function') self.settings.progress({percent: progress, buffered: self.el.buffered.end(0)});
				} catch(err) {
					//Do nothing!
				}
				if(progress == 100) {
	...
	function metaLoaded() {
		...
		/* Callback */
		if(typeof self.settings.onloadedmetadata === 'function') self.settings.onloadedmetadata({duration: self.duration});
	}
